# 分散システムデザインパターン
[Amazon](https://amzn.to/2LJTKO2)

## まとめ
- 前半はKubernetesを使ってどうシステムのデザインパターンを実現するかなど面白い内容でした
- 後半は特筆するようなこともないかな...という内容でした
- 「システムのアーキテクチャをKubernetesで表現するとこんな感じなのかぁ」というポイントが面白かったものの、量的にそれをここに載せられないので普通のアーキテクチャの解説になってしまいました
    - 気になった箇所は自分でCheck it out!
...
RedisをPrometheus(監視データ集約ツール)で監視するためのアダプタパターン
```yaml
apiVersion: v1
kind: Pod
metadata:    
